...
{
  //  NSLog(@"alphaForItemWithOffset=%f",offset);
	switch (type)
    {
...
{
  //  NSLog(@"valueForTransformOption=%f",value);
...
{	
   ...
   // NSLog(@"transformForItemView=%f",offset);
    //perform transform
    switch (type)
    {
		case iCarouselTypeCustom:
...
- (CGFloat)clampedOffset:(CGFloat)offset
{
    float zheng = ((int)offset)%566;
    float change = (zheng+(offset-((int)offset)))/567.666666;
    if ([delegate respondsToSelector:@selector(atTimeLinstenScroll:)]) {
        [delegate atTimeLinstenScroll:change];
    }
    if (shouldWrap)
    {
        if (numberOfItems == 0)
...
{
   //  NSLog(@"minScrollDistanceFromIndex");
...
{
    // NSLog(@"minScrollDistanceFromOffset");
...
#endif
#else
            CVDisplayLinkCreateWithActiveCGDisplays((void *)&self.timer);
        	CVDisplayLinkSetOutputCallback((__bridge CVDisplayLinkRef)self.timer, (CVDisplayLinkOutputCallback)&displayLinkCallback, (__bridge void *)self);
            CVDisplayLinkStart((__bridge CVDisplayLinkRef)self.timer);
#endif
        }
...
- (void)stopAnimation
{
    ...
    {
    	CVDisplayLinkStop((__bridge CVDisplayLinkRef)self.timer);
        CVDisplayLinkRelease((__bridge CVDisplayLinkRef)self.timer);
    }
